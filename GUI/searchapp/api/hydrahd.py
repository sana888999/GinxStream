# 21.04.26
# GUI API wrapper for the HydraHD service (hydrahd.ru).
...
from StreamingCommunity.services.hydrahd.scrapper import GetSerieInfo


class HydraHDAPI(BaseStreamingAPI):
    def __init__(self):
        super().__init__()
        self.site_name = "hydrahd"
        self._load_config()
    ...
    def get_series_metadata(self, media_item: Entries) -> Optional[List[Season]]:
        ...
        for season in scrape_serie.seasons_manager.seasons:
            ...
            print(f"[hydrahd] Season {season.number} ({season.name}): {len(episodes)} episodes")
