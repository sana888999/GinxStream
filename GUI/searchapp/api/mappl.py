...
from StreamingCommunity.services.mappl.scrapper import GetSerieInfo


class MapplAPI(BaseStreamingAPI):
    def __init__(self):
        super().__init__()
        self.site_name = "mappl"
        self._load_config()
        self._search_fn = None

    def _load_config(self):
        self.base_url = config_manager.domain.get(self.site_name, "full_url", default="https://mappl.tv")
        ...
        if database and hasattr(database, "media_list"):
            for element in list(database.media_list):
                ...
                results.append(Entries(
                    ...
                    provider_language=item_dict.get("provider_language", "en"),
                    raw_data=item_dict,
                ))
        return results
    ...
    def get_series_metadata(self, media_item: Entries) -> Optional[List[Season]]:
        ...
        for season in scrape_serie.seasons_manager.seasons:
            ...
            seasons.append(Season(number=season.number, episodes=episodes, name=season.name))
            print(f"[mappl] Season {season.number} ({season.name}): {len(episodes)} episodes")
